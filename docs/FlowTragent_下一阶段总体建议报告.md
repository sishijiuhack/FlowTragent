...
后续每个阶段完成时，不只跑单元测试，还要至少进行一次 Ubuntu + Kali 的实际流量验证。验证重点包括：

- 判断正确性：报告是否准确描述攻击意图、响应状态、成功/失败证据和证据缺口。
- 实时性：从 Kali 发起请求到 `/alerts` 出现告警和报告的延迟。
- 资源占用：capture worker、analyzer worker、NOVA-F 检索和可选 Ollama 是否造成明显阻塞。
- 误报控制：404、弱相似 CVE、扫描探测和真实成功利用之间是否能正确分级。
- 证据一致性：报告中的 evidence_id、时间线、PCAP、CSV、JSON、Mermaid/DOT 图谱是否互相对应。

建议每阶段保留一组固定验收用例：

```text
1. 失败利用：Kali 发送 cmd/wget/curl 参数，Ubuntu SimpleHTTP 返回 404。
   预期：Possible exploitation attempt / low confidence。

2. 明确探测：Kali 发送 Log4Shell/JNDI 风格 payload，但无成功响应和后续行为。
   预期：exploitation attempt，不应判定为成功利用。

3. 疑似成功：Ubuntu 侧补充 endpoint/process log，出现命令执行或外联证据。
   预期：Possible/Likely successful exploitation，置信度随证据增强。

4. C2 行为：Kali 产生周期性 HTTP/DNS/TCP beacon。
   预期：C2 Analysis 出现对应 finding，并进入 `/alerts`。

5. 正常流量：Kali 访问普通静态页面。
   预期：低风险或 skipped，不生成高危报告。
```

推送节奏也按阶段控制：

- 一个阶段内可以连续修改和本地测试，不需要每个小改都推送 GitHub。
- 阶段完成后，必须完成单元测试 + Ubuntu/Kali 实测验收，再统一提交并推送。
- 每次阶段推送前，确认不包含 `reports/`、`data/live/`、`data/index/`、真实 PCAP、DataCon 数据集、模型文件和 `libs/nova-f` 本地大文件变更。
- 后续默认不将 `docs/`、`.gitignore`、`.gitmodules` 纳入阶段推送范围；这些文件作为本地规划、记录或仓库元信息维护，除非明确要求同步，否则不提交到 GitHub。

这种节奏比“边写边推”更适合 FlowTragent 当前阶段：可以保留迭代速度，同时让 GitHub 上的提交更接近可运行、可解释、可回滚的阶段性成果。
